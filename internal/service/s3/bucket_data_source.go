...
func dataSourceAwsS3BucketRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("arn", arn)
	d.Set("bucket_domain_name", meta.(*client.AWSClient).PartitionHostname(fmt.Sprintf("%s.s3", bucket)))

	err = bucketLocation(meta.(*client.AWSClient), d, bucket)
	if err != nil {
...
func bucketLocation(client *client.AWSClient, d *schema.ResourceData, bucket string) error {
