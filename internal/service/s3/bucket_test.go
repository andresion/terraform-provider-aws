...
import (
	...
	tfcloudformation "github.com/terraform-providers/terraform-provider-aws/internal/service/cloudformation"
...
func testSweepS3Buckets(region string) error {
	...
	conn := client.(*client.AWSClient).S3Conn
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeAggregateTestCheckFunc(
			...
			func(s *terraform.State) error {
				// Tear down CF stack.
				conn := acctest.Provider.Meta().(*client.AWSClient).CloudFormationConn
				...
				if _, err := tfcloudformation.StackDeleted(conn, stackID, requestToken, 10*time.Minute); err != nil {
...
func TestAccAWSS3Bucket_Security_corsUpdate(t *testing.T) {
	...
	updateBucketCors := func(n string) resource.TestCheckFunc {
		return func(s *terraform.State) error {
			...
			conn := acctest.Provider.Meta().(*client.AWSClient).S3Conn
...
func TestAccAWSS3Bucket_Replication_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy:      acctest.CheckWithProviders(testAccCheckAWSS3BucketDestroyWithProvider, &providers),
		Steps: []resource.TestStep{
			{
				Config: testAccAWSS3BucketConfigReplication(rInt),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSS3BucketExistsWithProvider(resourceName, acctest.RegionProviderFunc(region, &providers)),
					testAccCheckAWSS3BucketExistsWithProvider("aws_s3_bucket.destination", acctest.RegionProviderFunc(alternateRegion, &providers)),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSS3BucketExistsWithProvider(resourceName, acctest.RegionProviderFunc(region, &providers)),
					testAccCheckAWSS3BucketExistsWithProvider("aws_s3_bucket.destination", acctest.RegionProviderFunc(alternateRegion, &providers)),
					testAccCheckAWSS3BucketExistsWithProvider("aws_s3_bucket.destination2", acctest.RegionProviderFunc(alternateRegion, &providers)),
					testAccCheckAWSS3BucketExistsWithProvider("aws_s3_bucket.destination3", acctest.RegionProviderFunc(alternateRegion, &providers)),
					resource.TestCheckResourceAttr(resourceName, "replication_configuration.#", "1"),
...
func TestWebsiteEndpoint(t *testing.T) {
	...
	for _, testCase := range testCases {
		got := WebsiteEndpoint(testCase.client.AWSClient, "bucket-name", testCase.LocationConstraint)
...
func testAccCheckAWSS3BucketDestroyWithProvider(s *terraform.State, provider *schema.Provider) error {
	conn := provider.Meta().(*client.AWSClient).S3Conn
...
func testAccCheckAWSS3BucketAddObjects(n string, keys ...string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).S3ConnURICleaningDisabled
  ...
  "Resources": {
    "TfTestBucket": {
      ...
      "Properties": {
		...
		stack, err := tfcloudformation.StackCreated(conn, aws.StringValue(resp.StackId), requestToken, 10*time.Minute)
		if err != nil {
...
func testAccCheckS3BucketDomainName(resourceName string, attributeName string, bucketName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		expectedValue := acctest.Provider.Meta().(*client.AWSClient).PartitionHostname(fmt.Sprintf("%s.s3", bucketName))
...
func testAccCheckS3BucketWebsiteEndpoint(resourceName string, attributeName string, bucketName string, region string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		website := WebsiteEndpoint(acctest.Provider.Meta().(*client.AWSClient), bucketName, region)
...
func testAccAWSS3BucketConfigSameRegionReplicationWithV2ConfigurationNoTags(rName, rNameDestination string) string {
	return acctest.ConfigCompose(testAccAWSS3BucketReplicationConfig_iamPolicy(rName), fmt.Sprintf(`
resource "aws_s3_bucket" "bucket" {
  bucket = %[1]q
  acl    = "private"
