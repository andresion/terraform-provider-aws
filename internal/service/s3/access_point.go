...
import (
	...
	tfs3control "github.com/terraform-providers/terraform-provider-aws/internal/service/s3control"
	"github.com/terraform-providers/terraform-provider-aws/internal/verify"
)

func ResourceS3AccessPoint() *schema.Resource {
...
func resourceAwsS3AccessPointCreate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).S3ControlConn
...
func resourceAwsS3AccessPointRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("domain_name", meta.(*client.AWSClient).RegionalHostname(fmt.Sprintf("%s-%s.s3-accesspoint", aws.StringValue(output.Name), accountId)))
