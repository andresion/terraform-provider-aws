...
func resourceAwsS3BucketCreate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).S3Conn
	...
	awsRegion := meta.(*client.AWSClient).Region
...
func resourceAwsS3BucketUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("tags_all") {
		...
		_, err := verify.RetryOnAWSCode(s3.ErrCodeNoSuchBucket, func() (interface{}, error) {
			terr := keyvaluetags.S3BucketUpdateTags(conn, d.Id(), o, n)
			return nil, terr
		})
		if err != nil {
...
func resourceAwsS3BucketRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("bucket_domain_name", meta.(*client.AWSClient).PartitionHostname(fmt.Sprintf("%s.s3", d.Get("bucket").(string))))
	...
	websiteEndpoint, err := websiteEndpoint(meta.(*client.AWSClient), d)
	if err != nil {
		return err
	}
	...
	tagsRaw, err := verify.RetryOnAWSCode(s3.ErrCodeNoSuchBucket, func() (interface{}, error) {
		return keyvaluetags.S3BucketListTags(conn, d.Id())
	})
...
func resourceAwsS3BucketDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if tfawserr.ErrMessageContains(err, "BucketNotEmpty", "") {
		if d.Get("force_destroy").(bool) {
			...
			conn = meta.(*client.AWSClient).S3ConnURICleaningDisabled
...
func websiteEndpoint(client *client.AWSClient, d *schema.ResourceData) (*S3Website, error) {
...
func WebsiteEndpoint(client *client.AWSClient, bucket string, region string) *S3Website {
	domain := WebsiteDomainUrl(client, region)
	return &S3Website{Endpoint: fmt.Sprintf("%s.%s", bucket, domain), Domain: domain}
}

func WebsiteDomainUrl(client *client.AWSClient, region string) string {
