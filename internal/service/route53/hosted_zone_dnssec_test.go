		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "signing_status", serveSignatureSigning),
				),
			},
			{
...
func TestAccAwsRoute53HostedZoneDnssec_SigningStatus(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAwsRoute53HostedZoneDnssecConfig_SigningStatus(rName, domainName, serveSignatureNotSigning),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccAwsRoute53HostedZoneDnssecExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "signing_status", serveSignatureNotSigning),
				),
			},
			{
				ResourceName:      resourceName,
			...
			{
				Config: testAccAwsRoute53HostedZoneDnssecConfig_SigningStatus(rName, domainName, serveSignatureSigning),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccAwsRoute53HostedZoneDnssecExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "signing_status", serveSignatureSigning),
				),
...
func testAccCheckAwsRoute53HostedZoneDnssecDestroy(s *terraform.State) error {
	conn := testAccProviderRoute53KeySigningKey.Meta().(*client.AWSClient).Route53Conn
	...
	for _, rs := range s.RootModule().Resources {
		...
		hostedZoneDnssec, err := findHostedZoneDNSSEC(conn, rs.Primary.ID)
		...
		if hostedZoneDnssec != nil && hostedZoneDnssec.Status != nil && aws.StringValue(hostedZoneDnssec.Status.ServeSignature) == serveSignatureSigning {
