		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "status", customDomainAssociationStatusPendingCertificateDNSValidation),
...
func testAccCheckAwsAppRunnerCustomDomainAssociationDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).AppRunnerConn

		domainName, serviceArn, err := customDomainAssociationParseID(rs.Primary.ID)

		if err != nil {
			return err
		}

		customDomain, err := findCustomDomain(context.Background(), conn, domainName, serviceArn)
