...
func testSweepAutoScalingPlansScalingPlans(region string) error {
	...
	conn := client.(*client.AWSClient).AutoScalingPlansConn
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAutoScalingPlansScalingPlanExists(resourceName, &scalingPlan),
					acctest.CheckResourceDisappears(acctest.Provider, ResourcePlansScalingPlan(), resourceName),
				),
...
func testAccCheckAutoScalingPlansScalingPlanDestroy(s *terraform.State) error {
	conn := acctest.Provider.Meta().(*client.AWSClient).AutoScalingPlansConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		scalingPlan, err := findScalingPlan(conn, rs.Primary.Attributes["name"], scalingPlanVersion)
		if err != nil {
			return err
		}
		if scalingPlan == nil {
