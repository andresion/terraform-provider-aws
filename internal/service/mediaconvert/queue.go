...
func resourceAwsMediaConvertQueueCreate(d *schema.ResourceData, meta interface{}) error {
	conn, err := getAwsMediaConvertAccountClient(meta.(*client.AWSClient))
	if err != nil {
...
func getAwsMediaConvertAccountClient(awsClient *client.AWSClient) (*mediaconvert.MediaConvert, error) {
	const mutexKey = `mediaconvertaccountconn`
	client.GlobalMutexKV.Lock(mutexKey)
	defer client.GlobalMutexKV.Unlock(mutexKey)
