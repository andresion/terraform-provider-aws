...
func testAccCheckAwsMediaConvertQueueDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		conn, err := getAwsMediaConvertAccountClient(acctest.Provider.Meta().(*client.AWSClient))
		if err != nil {
...
func testAccPreCheckAWSMediaConvert(t *testing.T) {
	_, err := getAwsMediaConvertAccountClient(acctest.Provider.Meta().(*client.AWSClient))
