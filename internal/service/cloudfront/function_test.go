...
func testSweepCloudfrontFunctions(region string) error {
	...
	conn := client.(*client.AWSClient).CloudFrontConn
...
func testAccCheckCloudfrontFunctionDestroy(s *terraform.State) error {
	conn := acctest.Provider.Meta().(*client.AWSClient).CloudFrontConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := findFunctionByNameAndStage(conn, rs.Primary.ID, cloudfront.FunctionStageDevelopment)
...
func testAccCheckAwsCloudfrontFunctionExists(n string, v *cloudfront.DescribeFunctionOutput) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		output, err := findFunctionByNameAndStage(conn, rs.Primary.ID, cloudfront.FunctionStageDevelopment)
