...
func resourceAwsLambdaPermissionCreate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).LambdaConn
	...
	client.GlobalMutexKV.Lock(functionName)
	defer client.GlobalMutexKV.Unlock(functionName)
...
func resourceAwsLambdaPermissionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if strings.HasPrefix(d.Get("function_name").(string), "arn:"+meta.(*client.AWSClient).Partition+":lambda:") {
		// Strip qualifier off
