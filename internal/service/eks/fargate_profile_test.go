...
func testSweepEksFargateProfiles(region string) error {
	...
	conn := client.(*client.AWSClient).EKSConn
...
func testAccCheckAWSEksFargateProfileExists(resourceName string, fargateProfile *eks.FargateProfile) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		clusterName, fargateProfileName, err := fargateProfileParseResourceID(rs.Primary.ID)

		if err != nil {
			return err
		}

		conn := acctest.Provider.Meta().(*client.AWSClient).EKSConn
...
func testAccCheckAWSEksFargateProfileDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err = findFargateProfileByClusterNameAndFargateProfileName(conn, clusterName, fargateProfileName)
...
func testAccPreCheckAWSEksFargateProfile(t *testing.T) {
	...
	region := acctest.Provider.Meta().(*client.AWSClient).Region
