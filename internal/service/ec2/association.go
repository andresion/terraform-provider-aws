...
func resourceAwsEipAssociationCreate(d *schema.ResourceData, meta interface{}) error {
	...
	supportedPlatforms := meta.(*client.AWSClient).SupportedPlatforms
	if len(supportedPlatforms) > 0 && !client.HasEC2Classic(supportedPlatforms) && resp.AssociationId == nil {
...
func resourceAwsEipAssociationRead(d *schema.ResourceData, meta interface{}) error {
	...
	request, err := describeAddressesById(d.Id(), meta.(*client.AWSClient).SupportedPlatforms)
	if err != nil {
		return err
	}
...
func resourceAwsEipAssociationDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if ip != nil {
		supportedPlatforms := meta.(*client.AWSClient).SupportedPlatforms
		if len(supportedPlatforms) > 0 && !client.HasEC2Classic(supportedPlatforms) {
