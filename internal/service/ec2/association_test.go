...
func testAccCheckAWSEIPAssociationExists(name string, res *ec2.Address) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		platforms := acctest.Provider.Meta().(*client.AWSClient).SupportedPlatforms
		...
		if len(describe.Addresses) != 1 ||
			(!client.HasEC2Classic(platforms) && *describe.Addresses[0].AssociationId != *res.AssociationId) {
...
func testAccCheckAWSEIPAssociationEc2ClassicExists(name string, res *ec2.Address) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := testAccProviderEc2Classic.Meta().(*client.AWSClient).EC2Conn
		platforms := testAccProviderEc2Classic.Meta().(*client.AWSClient).SupportedPlatforms
