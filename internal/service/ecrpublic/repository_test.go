...
func testSweepEcrPublicRepositories(region string) error {
	...
	conn := client.(*client.AWSClient).ECRPublicConn
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSEcrPublicRepositoryExists(resourceName, &v),
					acctest.CheckResourceDisappears(acctest.Provider, ResourcePublicRepository(), resourceName),
				),
...
func testAccCheckAWSEcrPublicRepositoryExists(name string, res *ecrpublic.Repository) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).ECRPublicConn
...
func testAccPreCheckAwsEcrPublic(t *testing.T) {
	...
	if region := acctest.Provider.Meta().(*client.AWSClient).Region; region != endpoints.UsEast1RegionID {
