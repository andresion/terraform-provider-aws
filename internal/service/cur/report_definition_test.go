...
func testSweepCurReportDefinitions(region string) error {
	c, err := acctest.SharedRegionalSweeperClient(region)
	if err != nil {
		return fmt.Errorf("error getting client: %s", err)
	}
	client := c.(*client.AWSClient)
...
func testAccCheckAwsCurReportDefinitionDestroy(s *terraform.State) error {
	conn := testAccProviderCur.Meta().(*client.AWSClient).CURConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		matchingReportDefinition, err := findReportDefinitionByName(conn, rs.Primary.ID)
		if err != nil {
