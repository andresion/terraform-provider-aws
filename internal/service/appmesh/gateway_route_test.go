...
func testSweepAppmeshGatewayRoutes(region string) error {
	...
	conn := client.(*client.AWSClient).AppMeshConn
...
func testAccCheckAppmeshGatewayRouteDestroy(s *terraform.State) error {
	conn := acctest.Provider.Meta().(*client.AWSClient).AppMeshConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := findGatewayRoute(conn, rs.Primary.Attributes["mesh_name"], rs.Primary.Attributes["virtual_gateway_name"], rs.Primary.Attributes["name"], rs.Primary.Attributes["mesh_owner"])
...
func testAccCheckAppmeshGatewayRouteExists(name string, v *appmesh.GatewayRouteData) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		out, err := findGatewayRoute(conn, rs.Primary.Attributes["mesh_name"], rs.Primary.Attributes["virtual_gateway_name"], rs.Primary.Attributes["name"], rs.Primary.Attributes["mesh_owner"])
		if err != nil {
			return err
		}
