...
func testSweepElasticSearchDomains(region string) error {
	...
	conn := client.(*client.AWSClient).ElasticSearchConn
...
func TestAccAWSElasticSearchDomain_duplicate(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy: func(s *terraform.State) error {
			conn := acctest.Provider.Meta().(*client.AWSClient).ElasticSearchConn
...
func testAccPreCheckIamServiceLinkedRoleEs(t *testing.T) {
	conn := acctest.Provider.Meta().(*client.AWSClient).IAMConn
	dnsSuffix := acctest.Provider.Meta().(*client.AWSClient).DNSSuffix
