...
func testSweepBudgetsBudgets(region string) error {
	...
	conn := client.(*client.AWSClient).BudgetsConn
	accountID := client.(*client.AWSClient).AccountID
...
func TestAccAWSBudgetsBudget_CostTypes(t *testing.T) {
	...
	startDate1 := timePeriodTimestampToString(&ts1)
	endDate1 := timePeriodTimestampToString(&ts2)
	startDate2 := timePeriodTimestampToString(&ts3)
	endDate2 := timePeriodTimestampToString(&ts4)
...
func testAccAWSBudgetsBudgetExists(resourceName string, v *budgets.Budget) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).BudgetsConn

		accountID, budgetName, err := budgetParseResourceID(rs.Primary.ID)

		if err != nil {
			return err
		}

		output, err := findBudgetByAccountIDAndBudgetName(conn, accountID, budgetName)
...
func testAccAWSBudgetsBudgetDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err = findBudgetByAccountIDAndBudgetName(conn, accountID, budgetName)
