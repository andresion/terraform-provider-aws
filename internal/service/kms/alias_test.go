		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "name", aliasNamePrefix+rName),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSKmsAliasExists(resourceName, &alias),
					resource.TestMatchResourceAttr(resourceName, "name", regexp.MustCompile(fmt.Sprintf("%s[[:xdigit:]]{%d}", aliasNamePrefix, resource.UniqueIDSuffixLength))),
					resource.TestCheckResourceAttr(resourceName, "name_prefix", aliasNamePrefix),
				),
			},
			{
				ResourceName:      resourceName,
...
func TestAccAWSKmsAlias_NamePrefix(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSKmsAliasConfigNamePrefix(rName, aliasNamePrefix+"tf-acc-test-prefix-"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSKmsAliasExists(resourceName, &alias),
					naming.TestCheckResourceAttrNameFromPrefix(resourceName, "name", aliasNamePrefix+"tf-acc-test-prefix-"),
					resource.TestCheckResourceAttr(resourceName, "name_prefix", aliasNamePrefix+"tf-acc-test-prefix-"),
				),
			},
			{
				ResourceName:      resourceName,
...
func testAccCheckAWSKmsAliasDestroy(s *terraform.State) error {
	conn := acctest.Provider.Meta().(*client.AWSClient).KMSConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := FindAliasByName(conn, rs.Primary.ID)
...
func testAccCheckAWSKmsAliasExists(name string, v *kms.AliasListEntry) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		output, err := FindAliasByName(conn, rs.Primary.ID)
