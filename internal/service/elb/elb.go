...
import (
	...
	tfec2 "github.com/terraform-providers/terraform-provider-aws/internal/service/ec2"
...
func resourceAwsElbCreate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).ELBConn
	...
	err = resource.Retry(5*time.Minute, func() *resource.RetryError {
		_, err := conn.CreateLoadBalancer(elbOpts)
...
func resourceAwsElbRead(d *schema.ResourceData, meta interface{}) error {
	...
	return flattenAwsELbResource(d, meta.(*client.AWSClient).EC2Conn, conn, describeResp.LoadBalancerDescriptions[0], ignoreTagsConfig, defaultTagsConfig)
}

// flattenAwsELbResource takes a *elbv2.LoadBalancer and populates all respective resource fields.
func flattenAwsELbResource(d *schema.ResourceData, conn *ec2.EC2, conn *elb.ELB, lb *elb.LoadBalancerDescription, ignoreTagsConfig *keyvaluetags.IgnoreConfig, defaultTagsConfig *keyvaluetags.DefaultConfig) error {
	...
	if lb.SourceSecurityGroup != nil {
		...
		if lb.VPCId != nil {
			sg, err := tfec2.FindSecurityGroupByNameAndVPCID(conn, aws.StringValue(lb.SourceSecurityGroup.GroupName), aws.StringValue(lb.VPCId))
			if err != nil {
...
func resourceAwsElbUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("listener") {
		...
		if len(remove) > 0 {
			...
			_, err := conn.DeleteLoadBalancerListeners(deleteListenersOpts)
			if err != nil {
		...
		if len(add) > 0 {
			...
			err := resource.Retry(5*time.Minute, func() *resource.RetryError {
				_, err := conn.CreateLoadBalancerListeners(input)
				if err != nil {
	...
	if d.HasChanges("cross_zone_load_balancing", "idle_timeout", "access_logs") {
		...
		_, err := conn.ModifyLoadBalancerAttributes(&attrs)
		if err != nil {
	...
	if d.HasChange("health_check") {
		...
		if len(hc) > 0 {
			...
			_, err := conn.ConfigureHealthCheck(&configureHealthCheckOpts)
			if err != nil {
	...
	if d.HasChange("security_groups") {
		...
		_, err := conn.ApplySecurityGroupsToLoadBalancer(&applySecurityGroupsOpts)
		if err != nil {
	...
	if d.HasChange("availability_zones") {
		...
		if len(added) > 0 {
			...
			_, err := conn.EnableAvailabilityZonesForLoadBalancer(enableOpts)
			if err != nil {
		...
		if len(removed) > 0 {
			...
			_, err := conn.DisableAvailabilityZonesForLoadBalancer(disableOpts)
			if err != nil {
	...
	if d.HasChange("subnets") {
		...
		if len(removed) > 0 {
			...
			_, err := conn.DetachLoadBalancerFromSubnets(detachOpts)
			if err != nil {
		...
		if len(added) > 0 {
			...
			err := resource.Retry(5*time.Minute, func() *resource.RetryError {
				_, err := conn.AttachLoadBalancerToSubnets(attachOpts)
				if err != nil {
...
func resourceAwsElbDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := cleanupELBNetworkInterfaces(meta.(*client.AWSClient).EC2Conn, name)
	if err != nil {
