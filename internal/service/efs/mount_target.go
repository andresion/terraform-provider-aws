...
func resourceAwsEfsMountTargetCreate(d *schema.ResourceData, meta interface{}) error {
	...
	az, err := getAzFromSubnetId(subnetId, meta.(*client.AWSClient).EC2Conn)
	if err != nil {
		return fmt.Errorf("Failed getting Availability Zone from subnet ID (%s): %s", subnetId, err)
	}
	mtKey := "efs-mt-" + fsId + "-" + az
	client.GlobalMutexKV.Lock(mtKey)
	defer client.GlobalMutexKV.Unlock(mtKey)
...
func resourceAwsEfsMountTargetRead(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("dns_name", meta.(*client.AWSClient).RegionalHostname(fmt.Sprintf("%s.efs", aws.StringValue(mt.FileSystemId))))
	d.Set("mount_target_dns_name", meta.(*client.AWSClient).RegionalHostname(fmt.Sprintf("%s.%s.efs", aws.StringValue(mt.AvailabilityZoneName), aws.StringValue(mt.FileSystemId))))
