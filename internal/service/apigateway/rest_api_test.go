...
func testSweepAPIGatewayRestApis(region string) error {
	...
	conn := client.(*client.AWSClient).APIGatewayConn
...
func TestAccAWSAPIGatewayRestApi_basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:     func() { acctest.PreCheck(t); testAccAPIGatewayTypeEDGEPreCheck(t) },
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					acctest.CheckResourceDisappears(acctest.Provider, ResourceRestAPI(), resourceName),
				),
		...
		Steps: []resource.TestStep{
			...
			// Test updating endpoint type
			{
				PreConfig: func() {
					...
					conn := acctest.Provider.Meta().(*client.AWSClient).APIGatewayConn
...
func testAccAPIGatewayTypeEDGEPreCheck(t *testing.T) {
	if acctest.Partition() != endpoints.AwsPartitionID {
		t.Skipf("skipping test; Endpoint Configuration type EDGE is not supported in this partition (%s)", acctest.Partition())
	}
}
