...
func testSweepDirectConnectGatewayAssociations(region string) error {
	...
	conn := client.(*client.AWSClient).DirectConnectConn
	...
	err = conn.DescribeTransitGatewaysPages(&ec2.DescribeTransitGatewaysInput{}, func(page *ec2.DescribeTransitGatewaysOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}
...
func testAccCheckAwsDxGatewayAssociationDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := findGatewayAssociationByID(conn, rs.Primary.Attributes["dx_gateway_association_id"])
...
func testAccCheckAwsDxGatewayAssociationExists(name string, ga *directconnect.GatewayAssociation, gap *directconnect.GatewayAssociationProposal) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		output, err := findGatewayAssociationByID(conn, rs.Primary.Attributes["dx_gateway_association_id"])
		...
		if proposalID := rs.Primary.Attributes["proposal_id"]; proposalID != "" {
			output, err := findGatewayAssociationProposalByID(conn, proposalID)
