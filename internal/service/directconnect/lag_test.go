...
func TestAccAWSDxLag_basic(t *testing.T) {
	lagName1 := fmt.Sprintf("tf-dx-lag-%s", sdkacctest.RandString(5))
	lagName2 := fmt.Sprintf("tf-dx-lag-%s", sdkacctest.RandString(5))
...
func TestAccAWSDxLag_tags(t *testing.T) {
	lagName := fmt.Sprintf("tf-dx-lag-%s", sdkacctest.RandString(5))
	resourceName := "aws_dx_lag.test"
