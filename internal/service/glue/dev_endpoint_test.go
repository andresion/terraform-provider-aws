...
func testSweepGlueDevEndpoint(region string) error {
	...
	conn := client.(*client.AWSClient).GlueConn
...
func TestAccGlueDevEndpoint_PublicKeys(t *testing.T) {
	...
	publicKey3, _, err := sdkacctest.RandSSHKeyPair(testAccDefaultEmailAddress)
	if err != nil {
		t.Fatalf("error generating random SSH key: %s", err)
	}
	publicKey4, _, err := sdkacctest.RandSSHKeyPair(testAccDefaultEmailAddress)
	if err != nil {
		t.Fatalf("error generating random SSH key: %s", err)
	}
...
func testAccCheckAWSGlueDevEndpointExists(resourceName string, endpoint *glue.DevEndpoint) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).GlueConn
...
func testAccGlueDevEndpointConfig_SubnetID_SecurityGroupIDs(rName string) string {
	return acctest.ConfigCompose(testAccAvailableAZsNoOptInConfig(), testAccGlueDevEndpointConfig_Base(rName), fmt.Sprintf(`
resource "aws_glue_dev_endpoint" "test" {
  name               = %[1]q
  role_arn           = aws_iam_role.test.arn
