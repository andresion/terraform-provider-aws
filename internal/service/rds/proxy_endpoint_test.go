		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSDBProxyEndpointExists(resourceName, &v),
					acctest.CheckResourceDisappears(acctest.Provider, ResourceDBProxyEndpoint(), resourceName),
				),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAWSDBProxyEndpointExists(resourceName, &v),
					acctest.CheckResourceDisappears(acctest.Provider, ResourceDBProxy(), "aws_db_proxy.test"),
				),
				ExpectNonEmptyPlan: true,
			},
		},
...
func testAccCheckAWSDBProxyEndpointDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		dbProxyEndpoint, err := findDBProxyEndpoint(conn, rs.Primary.ID)
