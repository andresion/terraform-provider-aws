...
func testSweepCognitoUserPoolDomains(region string) error {
	...
	conn := client.(*client.AWSClient).CognitoIdentityProviderConn
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckAWSCognitoUserPoolDomainExists(resourceName),
					acctest.CheckResourceDisappears(acctest.Provider, ResourceUserPoolDomain(), resourceName),
				),
...
func testAccCheckAWSCognitoUserPoolDomainExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).CognitoIdentityProviderConn
