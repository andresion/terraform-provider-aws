...
func testSweepCloudWatchEventPermissions(region string) error {
	...
	conn := client.(*client.AWSClient).CloudWatchEventsConn
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "event_bus_name", defaultEventBusName),
				),
			},
			{
...
func testAccCheckCloudWatchEventPermissionExists(pr string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		eventBusName, statementID, err := permissionParseID(rs.Primary.ID)
		if err != nil {
