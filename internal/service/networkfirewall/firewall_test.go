...
func testSweepNetworkFirewallFirewalls(region string) error {
	...
	conn := client.(*client.AWSClient).NetworkFirewallConn
	ctx := context.TODO()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAwsNetworkFirewallFirewallExists(resourceName),
					acctest.CheckResourceDisappears(acctest.Provider, ResourceFirewall(), resourceName),
				),
...
func testAccCheckAwsNetworkFirewallFirewallDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).NetworkFirewallConn
		output, err := findFirewall(context.Background(), conn, rs.Primary.ID)
