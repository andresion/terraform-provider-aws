...
func dataSourceAwsImageBuilderImagePipelineRead(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).ImageBuilderConn
	...
	d.Set("tags", keyvaluetags.ImagebuilderKeyValueTags(imagePipeline.Tags).IgnoreAws().IgnoreConfig(meta.(*client.AWSClient).IgnoreTagsConfig).Map())
