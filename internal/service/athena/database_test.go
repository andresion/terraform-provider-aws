...
func testAccCheckAWSAthenaDatabaseDestroy(s *terraform.State) error {
	conn := acctest.Provider.Meta().(*client.AWSClient).AthenaConn
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := conn.CreateBucket(&s3.CreateBucketInput{
		...
		ers, err := queryExecutionResult(*resp.QueryExecutionId, conn)
		if err != nil {
			return err
		}
		found := false
		...
		_, err = conn.DeleteObjects(&s3.DeleteObjectsInput{
			Bucket: aws.String(bucketName),
			Delete: &s3.Delete{
...
func testAccAWSAthenaDatabaseCreateTables(dbName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		_, err = queryExecutionResult(*resp.QueryExecutionId, conn)
		return err
	}
}
