...
func testAccCheckAWSSecurityHubStandardsControlExists(n string, control *securityhub.StandardsControl) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).SecurityHubConn

		standardsSubscriptionARN, err := standardsControlARNToStandardsSubscriptionARN(rs.Primary.ID)

		if err != nil {
			return err
		}

		output, err := findStandardsControlByStandardsSubscriptionARNAndStandardsControlARN(context.TODO(), conn, standardsSubscriptionARN, rs.Primary.ID)
