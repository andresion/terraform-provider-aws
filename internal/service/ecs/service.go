...
func resourceAwsEcsServiceCreate(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*client.AWSClient).ECSConn
...
func resourceAwsEcsServiceRead(d *schema.ResourceData, meta interface{}) error {
	...
	if service.TaskDefinition != nil {
		...
		if strings.HasPrefix(d.Get("task_definition").(string), "arn:"+meta.(*client.AWSClient).Partition+":ecs:") {
	...
	if strings.HasPrefix(d.Get("cluster").(string), "arn:"+meta.(*client.AWSClient).Partition+":ecs:") {
	...
	if service.RoleArn != nil {
		if strings.HasPrefix(d.Get("iam_role").(string), "arn:"+meta.(*client.AWSClient).Partition+":iam:") {
