...
func testSweepElasticBeanstalkEnvironments(region string) error {
	...
	conn := client.(*client.AWSClient).ElasticBeanstalkConn
...
func testAccCheckBeanstalkEnvExists(n string, app *elasticbeanstalk.EnvironmentDescription) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		env, err := describeBeanstalkEnv(acctest.Provider.Meta().(*client.AWSClient).ElasticBeanstalkConn, aws.String(rs.Primary.ID))
		if err != nil {
			return err
		}
