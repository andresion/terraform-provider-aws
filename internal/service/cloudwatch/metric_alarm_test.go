		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudWatchMetricAlarmExists(resourceName, &alarm),
					acctest.CheckResourceDisappears(acctest.Provider, ResourceMetricAlarm(), resourceName),
				),
...
func testAccCheckCloudWatchMetricAlarmExists(n string, alarm *cloudwatch.MetricAlarm) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*client.AWSClient).CloudWatchConn
		resp, err := findMetricAlarmByName(conn, rs.Primary.ID)
		if err != nil {
			return err
		}
		if resp == nil {
