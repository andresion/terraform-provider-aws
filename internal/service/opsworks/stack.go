...
func resourceAwsOpsworksStackRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*client.AWSClient).OpsWorksConn
...
func opsworksConnForRegion(region string, meta interface{}) (*opsworks.OpsWorks, error) {
	originalConn := meta.(*client.AWSClient).OpsWorksConn
	...
	sess.Handlers.Build.PushBack(request.MakeAddToUserAgentHandler("APN/1.0 HashiCorp/1.0 Terraform", meta.(*client.AWSClient).TerraformVersion))
